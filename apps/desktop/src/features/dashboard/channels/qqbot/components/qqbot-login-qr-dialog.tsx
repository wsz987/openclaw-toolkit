...
export type QqbotLoginQrDialogProps = {
  open: boolean;
  loading: boolean;
  polling: boolean;
  error: string | null;
...
export function QqbotLoginQrDialog({
  open,
  loading,
  polling,
  error,
      ...
      <AlertDialogContent className="w-[min(92vw,28rem)] gap-5 p-6">
        <AlertDialogHeader className="items-center text-center">
          ...
          <AlertDialogDescription className="max-w-[320px] text-xs leading-relaxed text-[hsl(var(--muted))]">
            请使用手机 QQ 扫描二维码并确认登录。登录成功后，前往 QQ 开放平台创建机器人并复制 AppID / AppSecret。
          </AlertDialogDescription>
        </AlertDialogHeader>
          ...
          <div className="relative flex h-[248px] w-[248px] items-center justify-center rounded-3xl border border-[hsl(var(--hairline))] bg-white p-4 shadow-sm">
            {loading ? (
              <div className="flex flex-col items-center gap-2">
                ...
                <span className="text-[11px] text-[hsl(var(--muted))]">正在生成 QQ 登录二维码...</span>
              </div>
            ) : renderedQrDataUrl ? (
              <>
                <img src={renderedQrDataUrl} alt="QQ Open Platform login QR code" className="h-[208px] w-[208px] rounded-2xl" />
                {isExpired ? (
                  <button
                    type="button"
                    onClick={onGenerate}
              ...
              <div className="flex max-w-[180px] flex-col items-center gap-3 text-center text-xs text-[hsl(var(--muted))]">
                <span>{error ?? '尚未生成二维码，请重新发起登录。'}</span>
