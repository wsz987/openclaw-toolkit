...
import { FEISHU_PERMISSION_TROUBLESHOOTING } from '../model/feishu-docs';

type FeishuHelpDialogProps = {
  open: boolean;
  copied: boolean;
  onCopy: () => void;
...
export function FeishuHelpDialog({
  open,
  copied,
  onCopy,
  onOpenChange,
  onOpenPermissions
}: FeishuHelpDialogProps) {
  return (
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent className="w-[min(92vw,38rem)] gap-5">
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>{FEISHU_PERMISSION_TROUBLESHOOTING.title}</AlertDialogDescription>
        </AlertDialogHeader>
        ...
        <div className="rounded-xl border border-[hsl(var(--hairline))] bg-[hsl(var(--surface-soft))] p-4">
          <div className="space-y-2 text-xs leading-relaxed text-[hsl(var(--body))]">
            {FEISHU_PERMISSION_TROUBLESHOOTING.steps.map((step, index) => (
        ...
        <div className="rounded-xl border border-[hsl(var(--hairline))] bg-[hsl(var(--canvas))] p-4">
          ...
          <pre className="mt-2 whitespace-pre-wrap break-words text-[11px] leading-relaxed text-[hsl(var(--body))]">
            {FEISHU_PERMISSION_TROUBLESHOOTING.copyText}
          </pre>
        </div>
