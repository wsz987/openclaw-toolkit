...
import type { WechatPluginPanelProps } from '../components/wechat-plugin-panel';
import { createWechatChannelController } from '../model/wechat-channel-controller';
import { useWechatPluginInstall } from './use-wechat-plugin-install';
import { useWechatPluginUninstall } from './use-wechat-plugin-uninstall';
import { useChannelActivation } from '../../shared/hooks/use-channel-activation';

type UseWechatChannelControlResult = {
  controller: ReturnType<typeof createWechatChannelController>;
  forceEnabled: boolean;
  pluginInstall: ReturnType<typeof useWechatPluginInstall>;
  pluginUninstall: ReturnType<typeof useWechatPluginUninstall>;
...
export function useWechatChannelControl(props: WechatPluginPanelProps): UseWechatChannelControlResult {
  const [forceEnabled, setForceEnabled] = useState(false);
  const pluginInstall = useWechatPluginInstall(props.result.configPath);
  const pluginUninstall = useWechatPluginUninstall(props.result.configPath);
