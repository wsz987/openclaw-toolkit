...
import {
  ...
  DINGTALK_PLUGIN_VERIFICATION_ITEMS,
  getDingtalkConsoleLinks
} from '../model/dingtalk-docs';

type DingtalkDocLinksCardProps = {
  clientId: string;
  activeStep?: 'credentials' | 'bot' | 'stream' | 'release' | null;
  onOpenUrl: (url: string) => Promise<unknown> | unknown;
  onOpenFaq: () => void;
};

export function DingtalkDocLinksCard({ clientId, activeStep, onOpenUrl, onOpenFaq }: DingtalkDocLinksCardProps) {
  const links = getDingtalkConsoleLinks(clientId);
  ...
  const isCredentialsAutoCompleted = clientId.trim().length > 6;
  ...
  return (
    <Card className="border-dashed border-[hsl(var(--hairline))] bg-[hsl(var(--surface-soft))] transition-all duration-300">
      <CardHeader className="flex flex-col sm:flex-row items-start justify-between gap-4 p-5 pb-1">
        <div className="space-y-1">
          ...
          <p className="text-[11px] leading-relaxed text-[hsl(var(--muted))]">
            文档、开发者后台配置入口和接入核对项都集中在这里，便于您边配置边核对。
          </p>
        </div>
        <div className="flex flex-wrap justify-end gap-2 w-full sm:w-auto">
          <Button
            ...
            onClick={() => void onOpenUrl(DINGTALK_PLUGIN_GUIDE_URL)}
          >
            <BookOpen className="mr-1.5 h-3.5 w-3.5" />
            安装指南
          </Button>
          <Button variant="secondary" className="h-8 px-3 text-[11px] font-medium" onClick={onOpenFaq}>
      ...
      <CardContent className="flex flex-col gap-3 p-5">
        {DINGTALK_PLUGIN_VERIFICATION_ITEMS.map((item) => {
          const consoleUrl =
            item.id === 'credentials'
              ? links.credentials
              : item.id === 'bot' || item.id === 'stream'
                ? links.bot
                : links.permissions;
                ...
                >
                  {stepStatus === 'success' ? (
                    ...
                    <div className={`w-4 h-4 rounded-full border flex items-center justify-center transition-colors ${isFocused ? 'border-[hsl(var(--primary)/0.6)] bg-[hsl(var(--primary)/0.05)]' : 'border-[hsl(var(--hairline))] hover:border-[hsl(var(--muted))]'
                      ...
                      <span className="text-[9px] text-[hsl(var(--muted))] font-bold">{stepNumber[item.id]}</span>
                    </div>
                  )}
                </div>
        ...
        <div className="rounded-xl border border-[hsl(var(--warning)/0.2)] bg-[hsl(var(--warning)/0.04)] px-4 py-3 text-[11px] leading-relaxed text-[hsl(var(--body))]">
          <strong className="block text-[hsl(var(--body-strong))]">{DINGTALK_PERMISSION_TROUBLESHOOTING.title}</strong>
          <div className="mt-1">
            建议先确认机器人能力与 Stream 接收模式已开启，再核对 Client ID / Client Secret，避免反复卡在同一个报错里。
          </div>
        </div>
      </CardContent>
    </Card>
