...
export async function inspectInstallDashboard(input: OpenClawInstallPayload): Promise<InstallDashboard> {
  const response = await invoke<InstallDashboard>('inspect_install_dashboard_command', { input });
  return normalizeDashboard(response);
}

export async function startOpenClawInstall(input: OpenClawInstallPayload): Promise<OpenClawInstallResult> {
  return invoke<OpenClawInstallResult>('start_openclaw_install', { input });
}

export async function readInstallLogTail(baseDir: string, maxLines = 200): Promise<InstallLogTail> {
