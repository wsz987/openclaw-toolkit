...
const sqlitePath = resolve(process.env.SQLITE_DB_PATH ?? '/data/update-server.sqlite');
...
const migrationsDir = resolve('apps/update-server/drizzle');
if (existsSync(migrationsDir)) {
  const applied = new Set(
    db.prepare('SELECT name FROM __openclaw_migrations').all().map((row) => row.name)
  );
...
const server = spawnSync('node', ['apps/update-server/.next/standalone/apps/update-server/server.js'], {
  stdio: 'inherit',
